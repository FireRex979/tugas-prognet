                ...
                <form action="{{ route('animal.update', $animal->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('nama') is-invalid @enderror"
                            placeholder="Masukkan Nama Hewan" name="nama" value="{{ old('nama') ?? $animal->nama }}">
                        @error('nama')
                    ...
                    <div class="form-group">
                        ...
                        <select name="suara" class="form-control @error('suara') is-invalid @enderror" id="">
                            @foreach ($suara as $item)
                                <option value="{{ $item->id }}" @if($animal->suara_id == $item->id) selected @endif>{{ $item->suara }}</option>
                            @endforeach
                        </select>
                        @error('suara')
                            <span class="invalid-feedback">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control @error('umur') is-invalid @enderror"
                            placeholder="Masukkan Umur Hewan" name="umur" value="{{ old('umur') ?? $animal->usia }}">
                        @error('umur')
                            <span class="invalid-feedback">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="form-group">
                        ...
                        <select name="kaki" class="form-control @error('kaki') is-invalid @enderror" id="">
                            <option value="2" @if($animal->jumlah_kaki == '2') selected @endif>2</option>
                            <option value="2" @if($animal->jumlah_kaki == '4') selected @endif>4</option>
                        </select>
                        @error('kaki')
                            <span class="invalid-feedback">
                                <strong>{{ $message }}</strong>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" id="" cols="30" rows="5">{{ old('deskripsi') ?? $animal->deskripsi }}</textarea>
                        @error('deskripsi')
                            <span class="invalid-feedback">
                                <strong>{{ $message }}</strong>
                            </span>
